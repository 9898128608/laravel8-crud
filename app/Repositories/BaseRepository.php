...
abstract class BaseRepository
{
    ...
    public function getByParams($params)
    {
        ...
        if (isset($params['rightjoin'])) {
            $joins = Arr::wrap($params['rightjoin']);
            foreach ($joins as $join) {
                $parts = explode(',', $join);
                $query->rightjoin($parts[0], $parts[1], $parts[2], $parts[3]);
            }
        }
        // add joins to main object
        ...
        if (isset($params['with_trashed'])) {
            if (isset($params['per_page']) && is_numeric($params['per_page'])) {
                ...
                $records = $query->withTrashed()->paginate($params['per_page']);
            } else {
    ...
    public function restoreUpdate($params)
    {
        if (isset($params['id'])) {
            $entity = $this->entity->withTrashed()->where(['id' => $params['id']])->first();
            $entity->update($params);
            return $entity;
        } else {
            $entity = $this->entity->create($params);
